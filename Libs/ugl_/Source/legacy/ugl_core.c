...
ugl_menu_t *ugl_get_menu_by_id( uint32_t ID ){
	ugl_menu_t* menu = UI_current_menu;
	while(menu != NULL){
		if(menu->ID == ID){
			return menu;
		}
		menu = menu->caller_ptr;
	}
	return NULL;
}

void ugl_process( void ){
	if(UI_current_menu->process_f != NULL){
		UI_current_menu->process_f();
	}
}
...
void ugl_render( void ){
	...
	if(UI_current_menu->render_f != NULL){
		UI_current_menu->render_f();
	}
}
